...
function PlayPageContent() {
  ...
  const [playerType, setPlayerType] = useState<
    "html5" | "youtube" | "dailymotion" | "iframe"
  >("html5");
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      ...
      try {
        ...
        } else if (movieData.videoUrls?.sourceType === "mega") {
          setPlayerType("iframe"); // MEGA uses iframe embed
  ...
  const getEmbedUrl = useCallback(() => {
    ...
    // Handle MEGA URLs
    if (videoUrls?.sourceType === "mega") {
      const fileId = videoUrls.mega || "";
      const key = videoUrls.key || "";
    ...
    if (videoUrls?.sourceType === "bunny") {
      ...
      if (
        videoUrls.original &&
        videoUrls.original.includes("iframe.mediadelivery.net")
      ) {
        return videoUrls.original;
      }
    }
    ...
    return (
      <div className="fixed inset-0 bg-black flex flex-col items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-400 mb-6">
            This movie doesn&apos;t have a playable video source
          </p>
          <button
            onClick={handleBack}
            className="px-6 py-2 bg-red-600 hover:bg-red-700 transition-colors rounded-md text-white font-medium"
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col">
      ...
      >
        ...
        {playerType === "html5" && (
          <div
            className={`absolute inset-0 flex flex-col justify-between p-4 bg-gradient-to-b from-black/70 via-transparent to-black/70 z-10 ${
              showControls ? "opacity-100" : "opacity-0"
            } transition-opacity duration-300`}
            onClick={() => togglePlay()}
          >
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  style={{
                    width: `${(isSeeking ? seekProgress : progress) * 100}%`,
                  }}
                ></div>
                ...
                <div
                  ...
                  style={{
                    left: `calc(${
                      (isSeeking ? seekProgress : progress) * 100
                    }% - 10px)`,
                    top: "0px",
                  }}
                ></div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="text-white text-sm">
                    {formatTime(
                      isSeeking ? seekProgress * duration : currentTime
                    )}{" "}
                    / {formatTime(duration)}
                  </div>
                </div>
